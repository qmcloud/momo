...
class ModuleBController extends Controller
{
    ...
    public function index()
    {
        return Admin::content(function (Content $content) {
            ...
            $content->header('添加模板B');
            $content->description('添加模板B');
    ...
    public function getView($id = 0)
    {
        ...
        return view('admin.module.moduleB',compact('goodsList')); //
    }
    ...
    protected function form($id = '')
    {
        return Admin::form(SpecialItem::class, function (Form $form) use ($id){
            ...
            if(empty($id)){
                $form->setTitle('添加模板B元素');
            }else{
                $form->setTitle('修改模板B元素');
            }
            $form->hidden('special_id')->value(Input::get('specialId',0)); // 默认是首页
            $form->hidden('item_type')->value('moduleB'); // 默认是首页
            ...
            $form->saved(function (Form $form) use ($id) {
                ...
                if(empty($id)){
                    return redirect(admin_base_path('module-b').'/'.$form->model()->id.'/edit');
                }else{
                    return back();
                }
...
EOT;
    ...
    protected function  getFormUrl($id){
        $url = '';
        if(!$id){
            $url = admin_base_path('module-b').'?module='.Input::get('module','').'&specialId='.Input::get('specialId',0);
        }else{
            $url = admin_base_path('module-b').'/'.$id.'?module='.Input::get('module','').'&specialId='.Input::get('specialId',0);
        }
        return $url;
    }
}
