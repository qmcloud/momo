    ...
    public function open($imgname) {
        ...
        if (!is_file($imgname)) {
           // throw new PhalApi_Exception_BadRequest(T('不存在的图像文件'));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'不支持的缩略图裁剪类型'));
								 exit();
        };
        ...
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            //throw new PhalApi_Exception_BadRequest(T('非法图像文件'));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'非法图像文件'));
								 exit();
        }
    ...
    public function save($imgname, $type = null, $quality = 80, $interlace = true) {
        ...
        if (empty($this->img)) {
            //throw new PhalApi_Exception_BadRequest(T('没有可以被保存的图像资源'));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'没有可以被保存的图像资源'));
								 exit();
        }
    ...
    public function width() {
        ...
        if (empty($this->img)) {
            //throw new PhalApi_Exception_BadRequest(T('没有指定图像资源'));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'没有指定图像资源'));
								 exit();
        }
    ...
    public function crop($w, $h, $x = 0, $y = 0, $width = null, $height = null) {
        ...
        if (empty($this->img)) {
            //throw new PhalApi_Exception_BadRequest(T('没有可以被裁剪的图像资源'));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'没有可以被裁剪的图像资源'));
								 exit();
        }
    ...
    public function thumb($width, $height, $type = self::IMAGE_THUMB_SCALE) {
        ...
        if (empty($this->img)) {
            //throw new PhalApi_Exception_BadRequest(T('没有可以被缩略的图像资源'));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'没有可以被缩略的图像资源'));
								 exit();
        }
    ...
    public function water($source, $locate = self::IMAGE_WATER_SOUTHEAST, $alpha = 80) {
        ...
        if (empty($this->img)) {
            //throw new PhalApi_Exception_BadRequest(T('没有可以被添加水印的图像资源'));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'没有可以被添加水印的图像资源'));
								 exit();
        }
        if (!is_file($source)) {
           // throw new PhalApi_Exception_BadRequest(T('水印图像不存在'));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'水印图像不存在'));
								 exit();
        }
        ...
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            //throw new PhalApi_Exception_BadRequest(T('非法水印文件'));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'非法水印文件'));
								 exit();
        }
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    //throw new PhalApi_Exception_BadRequest(T('不支持的水印位置类型'));
										echo json_encode(array("ret"=>0,'file'=>'','msg'=>'不支持的水印位置类型'));
								 exit();
                }
        }
    ...
    public function text($text, $font, $size, $color = '#00000000', $locate = self::IMAGE_WATER_SOUTHEAST, $offset = 0, $angle = 0) {
        ...
        if (empty($this->img)) {
            //throw new PhalApi_Exception_BadRequest(T('没有可以被写入文字的图像资源'));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'没有可以被写入文字的图像资源'));
								 exit();
        }
        if (!is_file($font)) {
           // throw new PhalApi_Exception_BadRequest(T("不存在的字体文件：{$font}"));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'不存在的字体文件：'));
								 exit();
        }
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    //throw new PhalApi_Exception_BadRequest(T('不支持的文字位置类型'));
										echo json_encode(array("ret"=>0,'file'=>'','msg'=>'不支持的文字位置类型'));
								 exit();
                }
        }
        ...
        if (is_string($color) && 0 === strpos($color, '#')) {
           ...
           //throw new PhalApi_Exception_BadRequest(T('错误的颜色值'));
						echo json_encode(array("ret"=>0,'file'=>'','msg'=>'错误的颜色值'));
								 exit();
        }
