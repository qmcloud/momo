<template>
  <div>
    ...
    >
      <el-table-column label="名称空间" prop="namespace" />
      ...
      <el-table-column label="Ports">
        <template slot-scope="scope">
          <el-tag v-for="(item, index) in scope.row.ports" :key="index">
            *:{{ item.backend.servicePort }}{{ item.path }}
          </el-tag>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    async loadData() {
      ...
      const res = await listIngress(data);
    ...
    async edit(row) {
      ...
      if (res.code == 200) {
        ...
        this.formData = {
          ...
          host: res.data.spec.rules[0].host,
        };
    ...
    async enterDialog() {
      this.$refs.form.validate(async (valid) => {
        if (valid) {
          ...
          switch (this.type) {
            case "create":
              res = await createIngress(this.formData);
              break;
            case "update":
              res = await updateIngress(this.formData);
              break;
            default:
              res = await createIngress(this.formData);
              break;
